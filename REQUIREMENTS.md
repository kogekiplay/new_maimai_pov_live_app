...
| 游戏区域   | 1080×1080  | 1:1 比例，居中放置（经真机测试从5:4调整） |
| 左侧面板   | 420×1080   | 公告+当前/下一首卡片               |
| 右侧面板   | 420×1080   | 点歌队列列表                     |
| 游戏区域偏移 | X=420, Y=0 | 居中偏移                       |
...
### 阶段一：画幅改造 ✅ 已完成（2026-05-25）

**已完成的工作：**

1. **Config.swift 修改**
   - outputWidth: 720 → 1920, outputHeight: 1440 → 1080
   - 新增 gameAreaRatio = 1.0（1:1游戏区域，经真机测试从5:4调整为1:1）
   - 新增计算属性：gameAreaWidth(1080), gameAreaHeight(1080), gameAreaX(420), gameAreaY(0)
   - cropVerticalOffset → cropHorizontalOffset（左右平移游戏画面）

2. **新增 CanvasComposer 模块**（3个文件）
   - `MaimaiPOV/Canvas/CanvasComposer.swift` — 16:9画布合成器
   - `MaimaiPOV/Canvas/CanvasUniforms.swift` — Shader uniforms结构体
   - `MaimaiPOV/Canvas/Shaders/Canvas.metal` — cropAndCompose单Pass Compute Shader

3. **BBoxTracker.swift 修改**
   - outputRatio(0.5) → cropRatio(1.0)，与输出分辨率解耦
   - 所有裁剪区域计算适配1:1比例，增加stabWidth clamp保护
   - 初始化/追踪/回中/空闲/reset 全部更新

4. **LivePipelineManager.swift 修改**
   - 新增 canvasComposer 属性
   - 管线编码优先使用 CanvasComposer（单Pass），保留 CropRenderer 作为 fallback
   - cropVerticalOffset → cropHorizontalOffset，影响cx而非cy
   - isCropActive 同时检查 canvasComposer 和 cropRenderer

5. **RTMPStreamManager.swift 修改**
   - 720p: 720×1440 → 1280×720 (16:9)
   - 1080p: 1080×2160 → 1920×1080 (16:9)

6. **Phase2View.swift / DebugOverlayView.swift / DebugInfoManager.swift**
   - VOffset → HOffset，cropVerticalOffset → cropHorizontalOffset
...
**已知限制（阶段一）：**
- Overlay和SongCard功能已禁用（原1:2竖屏设计不兼容16:9画布），将在阶段二/三以新面板形式重新实现
- 左右两侧目前为纯深色背景，无UI信息
...
**目标**：在左侧420×1080区域实现当前歌曲卡片、下一首卡片、公告区域

**技术方案**：
- 新增 LeftPanelRenderer（WKWebView + HTML模板渲染为Metal纹理）
- CanvasComposer 增加 encodePanel 方法，将面板纹理合成到画布左侧
- 面板纹理仅在数据变化时重新渲染，不每帧渲染

**需要开发的文件：**
- `MaimaiPOV/Canvas/LeftPanelRenderer.swift` — 左侧面板WebView渲染器
- `MaimaiPOV/Canvas/Templates/LeftPanelTemplate.swift` — 左侧面板HTML模板
- 修改 `CanvasComposer.swift` — 增加面板纹理合成
- 修改 `Canvas/Shaders/Canvas.metal` — 增加面板纹理采样逻辑（或新增Panel.metal）
- 修改 `LivePipelineManager.swift` — 接入面板渲染

### 阶段三：右侧面板 ⏳ 待开发

**目标**：在右侧420×1080区域实现点歌队列滚动列表

**技术方案**：
- 逐项WebView纹理 + Metal动画（推荐方案）
- 每个队列项独立渲染为纹理，Metal负责位置和动画
- 重排序只需移动位置，不需重渲染纹理
- 新歌加入时只渲染新项纹理

**需要开发的文件：**
- `MaimaiPOV/Canvas/RightPanelRenderer.swift` — 右侧面板渲染器
- `MaimaiPOV/Canvas/QueueItemRenderer.swift` — 队列项纹理渲染器
- `MaimaiPOV/Canvas/Templates/QueueItemTemplate.swift` — 队列项HTML模板
- 修改 `CanvasComposer.swift` — 增加队列项纹理合成和动画
- 修改 `LivePipelineManager.swift` — 接入队列渲染和动画更新
